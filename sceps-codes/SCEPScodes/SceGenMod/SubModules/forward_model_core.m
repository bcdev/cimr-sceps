...
function [ bts, emis, bts_harm_iso, bts_harm_cs1, bts_harm_cs2 ] = forward_model_core( atmos_input, surf_input, sensor_input, dir_input )
...
%= activating sea emission harmoncis if angle is 55 deg
%  as this is what the current sensor simulator ingests
%  to add the wind direction contribution as function 
%  of any azimuth angle to the TOA ocean
%  emission. Notice that full emission but only for the
%  given azimuth angle is still calculated as originally.
...
if do_sea
  ...
  if do_emi_sea
    ...
    [emis_sea, emis_sea_harmonics, refl_sea ] = forward_model_sea_surface( SST, SSS, OWS, OWD, rfreq, sensor_input.ZA, tra);
...
%= using ODL notation
%  there will be zero if no sea emission


bts_sea_harm_iso = [0 0 0 0];
bts_sea_harm_cs1 = [0 0 0 0];
bts_sea_harm_cs2 = [0 0 0 0];
...
if do_sea
  ...
  %= isotropic component wind direction independent
    ...
    bts_sea_harm_iso(1)  = (tbdown(1) * tra * refl_sea(1) + SST * tra * emis_sea_harmonics.evn ) + ( tra * emis_sea_harmonics.ev0 * ( SST - tbdown(1) ) );
    bts_sea_harm_iso(2)  = (tbdown(2) * tra * refl_sea(2) + SST * tra * emis_sea_harmonics.ehn ) + ( tra * emis_sea_harmonics.eh0 * ( SST - tbdown(2) ) );

   
    %= anysotropic components to be used inside the simulator
    %  with the sin and cos of relative wind direction

    bts_sea_harm_cs1(1) = tra * emis_sea_harmonics.ev1 * ( SST - tbdown(1) );
    bts_sea_harm_cs2(1) = tra * emis_sea_harmonics.ev2 * ( SST - tbdown(1) );

    bts_sea_harm_cs1(2) = tra * emis_sea_harmonics.ev1 * ( SST - tbdown(2) );
    bts_sea_harm_cs2(2) = tra * emis_sea_harmonics.ev2 * ( SST - tbdown(2) );

    bts_sea_harm_cs1(3) = tra * emis_sea_harmonics.e31 * ( SST - tbdown(3) );
    bts_sea_harm_cs2(3) = tra * emis_sea_harmonics.e32 * ( SST - tbdown(3) );

    bts_sea_harm_cs1(4) = tra * emis_sea_harmonics.e41 * ( SST - tbdown(4) );
    bts_sea_harm_cs2(4) = tra * emis_sea_harmonics.e42 * ( SST - tbdown(4) );
...
% same for harmonics, but only if just water, we do not 
% mix over ice or land

if pc_sea == 1

  bts_harm_iso = tbup +  pc_sea * bts_sea_harm_iso; 
  bts_harm_cs1 = pc_sea * bts_sea_harm_cs1; 
  bts_harm_cs2 = pc_sea * bts_sea_harm_cs2; 

else

  bts_harm_iso = -999*ones(1,4);
  bts_harm_cs1 = bts_harm_iso; 
  bts_harm_cs2 = bts_harm_iso; 
