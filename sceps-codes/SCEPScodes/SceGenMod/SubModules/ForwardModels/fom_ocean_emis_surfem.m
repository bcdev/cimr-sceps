...
%
% FORMAT 
%          [e, e_all, z, e_dt, e_ds, e_dw ] = 
%			fom_ocean_emis_surfem( sst, sss, ows, owd, freq, angle, trans)
%
% DESCRIPTION
%
%	Matlab script implementing the SURFEM-Ocean microwave sea surface
%	emissivity model, a neural network parameterisation of the 
%	new PARMIO physical reference emissivity model simulating
%	all Stokes components for channels in the range 0.5-700 GHz. 
%	The same model can be found ported to Fortan as part of
%	the RTTOV v13.2. Here is slightly modified wrt to the
%	RTTOV version to also output the internal emissivity components.
%
%	The inputs are row vector of dimensions 1 x number of cases (nc), the
%       outputs are matrices of dimensions 4 x nc, apart from the fields of 
%	e_all, which are vectors of dimension 1 x nc.
%        
% OUT   e        [-]		Sea surface emissivity vector with V-pol, H-pol, S3, and
%				S4 components
%
%       e_all    [-]		Structure with a break downs of the individual
%				surface emissivity components as described at
%				Kilic et al., 2023, Equation 6 and 7. 
%
%				For V-pol and H-pol emissivity, e_all contains the fields
%				evn, ev0, ev1, ev2, ehn, eh0, eh1, eh2, where total 
%				emissivity is given by:

%				ev = evn + ev0 + ev1 * cos(owd) + ev2 * cos(2*owd)
%				eh = ehn + eh0 + eh1 * cos(owd) + eh2 * cos(2*owd)
%				
%				with evn and ehn being the flat component, i.e.,
%				without wind dependence, and owd is the ocean wind
%				direccion.
%				
%				For the S3 and S4 emissivities, e_all contains e31, e32,
%				e41, and e42, where total emissivity is given by:   
%
%				e3  = e31 * sin(owd) + e32 * sin(2*owd) 
%				e4  = e41 * sin(owd) + e42 * sin(2*owd) 
%
%       z        [-]            Sea surface atmospheric reflectivity correction,
%				using the FASTEM calculation with the correction
%				parameterized as function of the atmopsheric transmitance.
%				The reflectivty is r = z * (1-e) 
%                               
...
%	owd	[degrees]	Relative ocean wind direction
%				NOTE: The wind direction always needs to be
%				      relative to a given direction, i.e. Here
%				      it follows thew ERA5 convention: Wind 
%				      direction increases clockwise such that a 
%				      northerly wind is 0°, an easterly wind is 90°, 
%				      a southerly wind is 180°, and a westerly 
%				      wind is 270°. See: 
...
% REFERENCE 
%
%	Kilic, L., Prigent, C., Jimenez, C., Turner, E., Hocking, J., English, 
%	S., et al. (2023). Development of the SURface Fast Emissivity Model for 
%	Ocean (SURFEM-Ocean) based on the PARMIO radiative transfer model. 
%	Earth and Space Science, 10, e2022EA002785. 
%	https://doi. org/10.1029/2022EA002785
%
% HISTORY
%
%	2022-05-30	lise.kilic(at)obspm.fr: created 
%
%	2024-06-15	carlos.jimenez(at)estellus.fr: adapted to output the
%			individual emissivity components and the path correction
%			instead of the feflectivity
%
%	2025-20-03	carlos.jimenez(at)estellus.fr: adding some code documentation
%
% PROJECT
%	Funded by: 
%       A Reference Quality Model For Ocean Surface Emissivity 
%       And Backscatter From The Microwave To The Infrared           
%                 https://www.issibern.ch/teams/oceansurfemiss/
%
%=============================================================================

function [e, e_all, z, e_dt, e_ds, e_dw ] = fom_ocean_emis_surfem( sst, sss, ows, owd, freq, angle, trans)

%=== checking inputs

if nargout >= 3 & nargin < 7
 error('Calculation of z demanded but no transmission given')
end
...
if nargout <= 2
  [ e, e_all ] = SURFEM_ocean_K( freq, angle, sst, sss, ows, owd);
else
  [ e, e_all, e_dt, e_ds, e_dw ] = SURFEM_ocean_K( freq, angle, sst, sss, ows, owd);
end
...
if nargin == 7
  ...
  z = [ z_v; z_h; zeros(size(z_v)); zeros(size(z_v)) ];  
  % showing how to calculate reflectivty
  %     r = [ z_v .* (1-e(1,:)); z_h .* (1-e(2,:)); 
...
function [e,e_all,e_dt,e_ds,e_dw] = SURFEM_ocean_K(freq_vec,theta_vec,SST_vec, SSS_vec, OWS_vec,phi_vec)
...
for ii=1:len
  ...
  %=== if the SSS and SST limits are overpassed we 
  %    reppoyt output with nans so next
  %    calculations will carry the nan and likely
  %    resut in nan Tbs or inversions 
...
%---Structure with all e components
...
%------------------------------------------------------------------------
%
% For the restitution of the sea surface reflectivity with SURFEM we adopt the
% same path correction than FASTEM.
% 
% This produces a reflectivity "correction" meaning that the reflectivity
% [ rV rH rS3 rS4] for a emissiviyt vector [ eV eH eS3 eS4 ] needs to be
%  calculated as: 
%
%    rV  = zreflmod_v * (1-eV)
%    rH  = zreflmod_h * (1-eH)
...
%    rS4 = 0
%
% with zreflmod_v and zreflmod_h acomputed using the paramaterization 
% applied in FASTEM
%
%------------------------------------------------------------------------
