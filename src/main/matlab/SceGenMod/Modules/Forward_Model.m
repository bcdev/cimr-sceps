...
global SGM_NETCDF_SAVE_FILE


% making global whether we get TOA BTs or surface BTs
global ATMOSPHERE_ADDING
...
% get geoinout module
global GEOINPUT_SIMULATION
if isempty( GEOINPUT_SIMULATION )
  osfi_error('GEOINPUT_SIMULATION is not defined, you did not run Geoinputs_Extract before? Please define as GLOBAL as it would have been generated by that module');
end

% get scene date
global SCENE_DATE
if isempty( SCENE_DATE )
  osfi_error('SCENE_DATE is not defined, you did not run Geoinputs_Extract before? Please define as GLOBAL as it would have been generated by that module');
end

% get scene type
global SCENE_TYPE
if isempty( SCENE_TYPE )
  osfi_error('SCENE_TYPE is not defined, you did not run Geoinputs_Extract before? Please define as GLOBAL as it would have been generated by that module');
end
...
V_POL             = v_polarization;
H_POL             = h_polarization;
ATMOSPHERE_ADDING = atmosphere_adding;
...
if length( data_thinning ) == 1

 if data_thinning >= 1

  if strcmp( SCENE_TYPE, 'global' ) | strcmp( SCENE_TYPE, 'geo-nemo' ) | strcmp( SCENE_TYPE, 'geo-tropical' )
  ...
  elseif strcmp( SCENE_TYPE, 'polar' )

    osfi_error('Polar scenes need to be better tested');
    pro = 1:data_thinning:length(I.LAT);

  end

 end

elseif length(data_thinning) == 2

  %= new quick fix to do lat bads
  %  to be better implemented

  osfi_error('Data thinning as a 2-element vector needs to be better tested');
  ...
  if strcmp( SCENE_TYPE, 'global' )  | strcmp( SCENE_TYPE, 'geo-nemo' ) | strcmp( SCENE_TYPE, 'geo-tropical' )
    ...
    try  
      ...
      pro = reshape(1:length(I.LAT), no, na);
      pro = pro(:,data_thinning(1):data_thinning(2));
      ro  = size( pro,1);
      ra  = size( pro,2);
      pro = pro(:);
...
if workers_number > 0 
  ...
  parfor co = ico
    ...
    [ y, esea, eice, elan ]  = module_parallel( co, pro, jnd, zenith_angle, azimuth_angle, Q, I, LOG );
...
%=== Also saving lat, lon and UTC and PRO index after data thinning
...
%= UTC date number number of days from January 0, 0000
data    = single(I.UTC(pro));
sparam  = [ 'UTC'];
outfile = [ dirout, '/', idfunction, '_Output_', SCENE_DATE, '_', sparam, '.mat' ];  
LOG.info( [ idfunction, ' ** Saving ', sparam, ' fields for day ', SCENE_DATE ]);
save_data_single( outfile, data );
...
function [ bts, esea, eice, elan ]  = module_parallel( co, pro, dopro, zenith_angle, azimuth_angle, Q, I, LOG );
  ...
  if isempty(find(isnan(Q.atmos_input.T)==1))
    ...
    try 
	  ...
	  %  interpolate with log10(P)
          ...
          [ R, bts(za,aa,:)] = forward_model( Q, R, [], 1 );
